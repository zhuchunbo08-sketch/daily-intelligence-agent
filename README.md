...
《每日商业观察系统》是一个长期运行的 AI 情报自动化项目。系统每天北京时间 07:00 自动抓取昨天 07:00 到今天 07:00 的公开资讯，去重、过滤、分析后生成日报并推送到飞书。

第一版不做微信外挂、不读 Cookie、不模拟点击、不绕过平台风控。

日报固定的是目标，不是结构：让用户知道世界发生了什么重要变化，理解变化背后的规律，看到未来可能出现的新需求，并提升商业判断和认知边界。当天没有高价值内容时，只输出“今天没有发现值得占用你时间的重要变化。”，不为了填满模块而凑内容。
...
- AI 分类、全球变化识别、风险识别、深度理解、认知升级提炼。
- 政治政策、科技、商业、经济、机会观察分类。
- 明显割韭菜内容过滤。
- 动态结构 Markdown 日报。
- 世界变化数据库，记录趋势、判断、证据和后续验证状态。
- 飞书机器人 Webhook 推送。
- 飞书签名支持。
- 飞书消息过长自动分段。
- 邮件备用通道。
